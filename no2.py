...
score = 0
speed = 1  # Slow down the falling circles speed
bullet_speed = 30  # Increased speed for the projectiles
spawn_rate = 50 
# Draw functions
...
def update_falling_circles():
    ...
    for circle in falling_circles[:]:
        circle[1] -= speed  # Move circle downwards (slower speed)
...
def spawn_falling_circle():
    """Periodically spawn falling circles at random positions."""
    if random.randint(1, spawn_rate) == 1:  # Spawn a circle with a certain probability
        x_pos = random.randint(0, 450)  # Random x position
        falling_circles.append([x_pos, 600])  # Start falling from top
...
def display():
    ...
    display_buttons()
    spawn_falling_circle()  # Spawn falling circles periodically
    glutSwapBuffers()
...
def handle_keypress(key, x, y):
    ...
    global projectiles, catcher_x, game_over
    if game_over:
        return  # Don't allow movement or firing when the game is over
    if key == b' ':  # Spacebar to shoot
        projectile = [catcher_x + 25, 50]  # Initial projectile position
        projectiles.append(projectile)  # Add new projectile to the list
...
def reset_game():
    ...
    speed = .5  # Reset falling circle speed
